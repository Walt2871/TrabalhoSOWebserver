...
public class ThreadsDeConexao implements Runnable
{
    ...
    public void run(){
        try{
            ...
            File f = new File("/teatro/TrabalhoSOWebserver/arquivos_html/" + recurso);
            ByteArrayOutputStream bout = new ByteArrayOutputStream();
            if(!f.exists()){
                out.write("404 NOT FOUND\n\n".getBytes(StandardCharsets.UTF_8));
            }
            else{
                InputStream fileIn = new FileInputStream("/teatro/TrabalhoSOWebserver/arquivos_html/" + recurso);
                //escreve arquivos
